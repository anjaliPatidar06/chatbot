...
export default {
  chatDataOfUser: state => id => {
    // console.log('hello chat module getter',state.chats ,id)
  ...
  chatContacts: (state, getters) => {
    ...
    const chatContacts = state.chatContacts.filter((contact) => contact.contact.toLowerCase().includes(state.chatSearchQuery.toLowerCase()))

    chatContacts.sort((x, y) => {
      const timeX = getters.chatLastMessaged(x.uid).msg_time
      const timeY = getters.chatLastMessaged(y.uid).msg_time
  ...
  contacts: (state) => state.contacts.filter((contact) => contact.contact.toLowerCase().includes(state.chatSearchQuery.toLowerCase())),
