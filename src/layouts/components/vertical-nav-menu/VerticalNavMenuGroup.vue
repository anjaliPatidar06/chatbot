...
>
      ...
      <!-- Group Label -->
      <div @click="clickGroup" class="group-header w-full" v-show="accessNotAllowed(group)"
...
export default {
  ...
  methods: {
    accessNotAllowed(group) {
      console.log(localStorage.user_role,'localStorage.user_role')
      if(localStorage.user_role !== 'Superadmin') {
  ...
  mounted() {
    // console.log(this.$route,'open mounted')
