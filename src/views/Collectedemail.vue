...
export default {
  ...
  mounted() {
    ...
    console.log(this.chatbotname);
    // this.getFormData()
    if(localStorage.chatbotname !== undefined && localStorage.chatbotname !== null){
      console.log(typeof localStorage.chatbotname,'inside if')     
      this.$vs.loading({
  ...
  methods:{
    ...
    // },
    collectedLeads() {
        axios.post(Base_URL.Actual_URL + "leadtable", {
      ...
      .then((response) => {
        ...
        console.log(response ,'lead table response');
        this.rowdatanew = response.data.userlist;

        // console.log(this.rowdatanew);
      });
    }
  }
};
