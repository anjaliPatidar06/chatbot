...
<template>
  <div class="vx-row">
    ...
    <div class="vx-col w-full mb-base">
      <vx-card title="Update Synonyms">
        <div class="vx-row">
          <div class="vx-col sm:w-1/2 w-full mb-2" :data="rowdata">
            <vs-input
              class="w-full"
              label="Synonyms Name"
              v-model="rowdata[0].synonymsname"
            v-validate="'required'"
              name="synonym_name"/>
          ...
          <div class="vx-col sm:w-1/2 w-full mb-2" :data="rowdata">
            <vs-input
              class="w-full"
              label="Synonyms Word"
              v-model="rowdata[0].synonymsword"
             v-validate="'required'"
              name="synonym_word"/>
               <span class="text-danger text-sm">{{
...
export default {
  ...
  computed: {
    validateForm() {
      return (
        !this.errors.any() &&
        this.rowdata[0].synonymsname.length > 0 &&
        this.rowdata[0].synonymsword.length > 0
      );
    },
  },
  mounted() {
    const url = Base_URL.Actual_URL + "editsynonymname/";
  ...
  methods: {
    Update() {
      ...
      axios
        .post(url1, {
          synonymsname: this.rowdata[0].synonymsname,
          synonymsword: this.rowdata[0].synonymsword,
        })
        .then((response) => {
          ...
          if (response.data.code == 200) {
            ...
            this.$vs.notify({
              title: "Edit Synonym.",
              text: "The Synonym data is updated",
              color: "success",
              position: "top-center",
            });
            EventBus.$emit("editSynonymComponent");
            this.$router.push({
              name: "userTraining",
              params: { name: 'synonym'}
            });
