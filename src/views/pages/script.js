...
// var header_drop = '<div class="resp_dropdown"><form id="myForm"><select id="a"><option value="volvo">Volvo</option><option value="saab">Saab</option><option value="opel">Opel</option><option value="audi">Audi</option></select></form></div>'
// $(header_drop).appendTo(".chat_header")

//initialization
$(document).ready(function() {

	res_status = 'Rasa'
	//res_status = "LiveChatting"
	//Bot pop-up intro
    ...
    if (typeof socket == 'undefined'){
    	...
    	//socket.emit( 'my event', {data: 'Chatting connected'} )
    ...
    if( msg.user_name !== user_id && typeof msg.user_name !== 'undefined' && msg.message.length >= 1) {
    	//alert(msg.message.length)
        var BotResponse = '<img class="botAvatar" src="./static/img/sara_avatar.png"/><p class="botMsg">' + msg.message + '</p><div class="clearfix"></div>';
        hideBotTyping();
...
//=====================================    Fall back custom actions =====================

function action_fallback() {
    ...
    $.ajax({
        ...
        type: "POST",
        contentType: "application/json",
        data: JSON.stringify({ "name": utter_my_fallback_action, "policy": "MappingPolicy", "confidence": "0.98" }),
        success: function(botResponse, status) {
            console.log("Response from Rasa: ", botResponse, "\nStatus: ", status);
...
$(".usrInput").on("keyup keypress", function(e) {
    ...
    if (keyCode === 13) {
        ...
        if (text == "" || $.trim(text) == "") {
        	...
        	else if (res_status == 'LiveChatting') {
        	Send_socket(text);
        	}
...
function setUserResponse(message) {
    var UserResponse = '<div class="user_msg" id="user_msg"><img class="userAvatar" src=' + "./static/img/userAvatar.jpg" + '><p class="userMsg">' + message + ' </p><button class="btn1" id = "fa_trash"><i class="fa fa-trash" ></i></button><div class="clearfix"></div></div>';
...
//============== Set Current Times below every Bots message's =============================================
...
function setbotDate() {
    ...
    var BotResponse = '<div class="timestamp">' + hours + ':' + minutes + ' '+ ampm + '</div> <br>' ;
    //$('<div class="timestamp">' + hours + ':' + minutes + '</div>').appendTo(".chats");
    $(BotResponse).appendTo(".chats")
    }


//============== Set Current Times below user every message's =============================================
...
function setuserDate() {
    ...
    hours = hours ? hours : 12; // the hour '0' should be '12'
    minutes = minutes < 10 ? '0'+minutes : minutes;
    var BotResponse = '<div class="usertimestamp">' + hours + ':' + minutes + ' '+ ampm + '</div> <br>' ;
    //$('<div class="timestamp">' + hours + ':' + minutes + '</div>').appendTo(".chats");
    $(BotResponse).appendTo(".chats")
    }
...
function send(message) {
    ...
    //alert(message)
    if (message == 'changeresstatus'){
    	res_status = "LiveChatting"
    	Send_socket(message);
    	live_message = [{"text":"Please wait our Agent will connect to you soon..."}]
    	setBotResponse(live_message);
    	return
    }
    $.ajax({
...
function hyper_link(inptext) {
  
  var outtext;
  inptext = inptext.replace(/\n/g,"<br>");
  inptext = inptext.replace(/\\n/g,"<br>");
  
  var Word_arr = inptext.match(/\[(.*?)\]/g);
  var link_arr = inptext.match(/\(([^)]+)\)/g);
  
  if (Word_arr){
  for (n = 0; n < Word_arr.length; n++) {
      
      var str2 = String(Word_arr[n]);
      var str2 = str2.slice(1,-1);
  
      var link2 = String(link_arr[n]);
      var link2 = link2.slice(1,-1);
      
    var hyper = str2.link(link2);
    //alert(Word_arr[i]+link_arr[i]);
    var outtext = inptext.replace(Word_arr[n]+link_arr[n]," "+hyper+" ");
    //text = text.concat(hyper);
    var outtext = outtext.concat(" ");
    return outtext
    }
  }
    ...
    setTimeout(function() {
        ...
        if (response.length < 1) {
            ...
            for (i = 0; i < response.length; i++) {
                ...
                //var copy_respo =  response[i];
                //check if the response contains "text"
                var aSound = document.createElement('audio');
                aSound.setAttribute('src', './static/audio/beep.wav');
                aSound.play();
                if (response[i].hasOwnProperty("text")) {

                    //text = response[i].text;
                    var BotResponse = '<div class="Bot_Messages" id="Bot_Messages"><img class="botAvatar" src="./static/img/sara_avatar.png"/><p class="botMsg" id="bot_msg">' +hyper_link(response[i].text)+ '</p><button class="btn" id = "fa_trash_1"><i class="fa fa-trash" ></i></button><div class="clearfix"></div></div>';
                    $(BotResponse).appendTo(".chats").hide().fadeIn(1000);
                 	
					//alert(i);
                    //var option = '<button class="btn"><i class="fa fa-trash"></i></button><button class="btn"><i class="fa fa-reply"></i></button>'
                    //$(option).appendTo(".chats").hide().fadeIn(1000);
                }
...
$(document).on("click","#fa_trash", "#user_msg" , function(){
   alert("delete");
   console.log("Delete....");
...
function renderDropDwon(data) {
    ...
    for (i = 0; i < data.length; i++) {
        ...
        options += "<option value='" + String(data[i].value) + "'>" + data[i].label + "</option>"
    }
    
    
    var select = '<div class="dropDownMsg"><select class="browser-default dropDownSelect"> <option value="" disabled selected>Select a Option</option>' + options + '</select></div>'
    $(".chats").append(select);
    scrollToBottomOfResults();
...
function createCardsCarousel(cardsData) {
    ...
    for (i = 0; i < cardsData.length; i++) {
        ...
        //item = '<div class="carousel_cards in-left"><img class="cardBackgroundImage" id ="'+ cardsData[i].payload+'" onclick="send(this.id)" src="' + cardsData[i].image +'">'+'<div class="cardFooter">' + '<span class="cardTitle" title="' + title + '">' + title + "</span> " +  "</div>" + "</div>" ;
        //+'<div class="buttonBelow">'+'<button class="button button1">' + title +'</button>' +"</div>" 
        

        item = '<div class="card"><img class="card-img-top" src="'+ cardsData[i].image +'" alt="Card image cap"><div class="card-body"><h5 class="card-title">Card title</h5><p class="card-text">' +title+'</p></div><ul class="list-group list-group-flush"><li class="list-group-item text-center">Cras justo odio</li><li class="list-group-item text-center">Dapibus ac facilisis in</li><li class="list-group-item text-center"><a href="#" class="btn btn-primary">Full profile</a></li></ul><div class="card-body text-center"><botton>Add Button</botton><a href="#" class="card-link text-center">Card link</a></div></div>'
...
function showBotTyping() {
    ...
    var botTyping = '<img class="botAvatar" id="botAvatar" src="./static/img/sara_avatar.png"/><div class="botTyping">' + "Bot is Typing " +'<div class="bounce1"></div>' + '<div class="bounce2"></div>' + '<div class="bounce3"></div>' + '</div>'
    $(botTyping).appendTo(".chats");
    $('.botTyping').show();
    scrollToBottomOfResults();

    // if (msg.user_name !== user_id){
...
function showAgentTyping() {
    ...
    var botTyping = '<img class="botAvatar" id="botAvatar" src="./static/img/sara_avatar.png"/><div class="botTyping">' + "Agent is Typing " +'<div class="bounce1"></div>' + '<div class="bounce2"></div>' + '<div class="bounce3"></div>' + '</div>'
    $(botTyping).appendTo(".chats");
    $('.botTyping').show();
    scrollToBottomOfResults();
